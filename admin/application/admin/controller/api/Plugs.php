...
class Plugs extends Controller
{
    ...
    public function upload()
    {
		...
		if (!$file->checkSize(sysconf('storage_maxsize'))) {
            return json(['uploaded' => false, 'error' => ['message' => '文件上传异常，文件不能过大！('.sysconf('storage_maxsize_title').')']]);
        }
		//大小限制--最小
		if (!$file->checkSizeMin(sysconf('storage_minsize'))) {
            return json(['uploaded' => false, 'error' => ['message' => '文件上传异常，文件不能过小！('.sysconf('storage_minsize_title').')']]);
        }
		//类型限制
        ...
        if (is_array($info) && isset($info['url'])) {
           $this->success('文件上传成功！',['uploaded' => true, 'filename' => $name, 'url' => $this->safe ? $name : $info['url']]);
        }
		/*
        // 唯一名称

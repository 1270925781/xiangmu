...
class Base extends Controller
{
    ...
    public function __construct(){
        ...
        $tokenInfo = Db::name('StoreToken')->where(['token' => $token,'user_type'=>1])->find();
        if(empty($tokenInfo)){
            echo json_encode(array('code'=>-6));die;
        }
        ...
        $userInfo = Db::name('SystemUser')->where(['id'=>$tokenInfo['user_id']])->find();
        $this->uid = $tokenInfo['user_id'];
        $this->shopId = $userInfo['shop_id'];
    }

    //统一输出函数
    function showMsgBak($code=0,$msg='',$data=[]){
        $outdata=array(
            'code'=>$code,
            'msg'=>$msg,
            'data'=>$data
   ...
   function verifySms($phone, $code, $type){
     ...
     * 操作内容:content 
     **/
    function UserOperation($user_id,$content,$platform_type = 1){
